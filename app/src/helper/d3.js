...
const e10 = Math.sqrt(50);
  ...
  function tickStep(start, stop, count) {
    let step0 = Math.abs(stop - start) / Math.max(0, count);
      var step1 = 10**Math.floor(Math.log(step0) / Math.LN10);
  ...
  function histogram() {
    let value = identity;
      var domain = extent;
      var threshold = thresholdSturges;
  ...
  function scott(values, min, max) {
    return Math.ceil(
      (max - min) / (3.5 * deviation(values) * values.length**(-1 / 3))
    );
  }
...
const top = 1;
  ...
  function axis(orient, scale) {
    ...
    const values = tickValues == null ? (scale.ticks ? scale.ticks.apply(scale, tickArguments) : scale.domain()) : tickValues;
      ...
      var position = (scale.bandwidth ? center : number$1)(scale.copy());
...
const keyPrefix = "$"; // Protect against keys like “__proto__”.
  ...
  const list = classList(node); let i = -1; let n = names.length;
  ...
  if (typeof document !== "undefined") {
    let element$1 = document.documentElement;
  ...
  function contextListener(listener, index, group) {
    return function (event1) {
      let event0 = exports.event; // Events can be reentrant (e.g., focus).
  ...
  function onAdd(typename, value, capture) {
    let wrap = filterEvents.hasOwnProperty(typename.type)
...
const root = [null];
  ...
  function dragDisable(view) {
    let root = view.document.documentElement;
      var selection$$1 = select(view).on("dragstart.drag", noevent, true);
  ...
  const root = view.document.documentElement;
    var selection$$1 = select(view).on("dragstart.drag", null);
    if (noclick) {
  ...
  DragEvent.prototype.on = function () {
    let value = this._.on.apply(this._, arguments);
  ...
  function drag() {
    ...
    function mousemoved() {
      noevent();
      if (!mousemoving) {
        let dx = exports.event.clientX - mousedownx;
          var dy = exports.event.clientY - mousedowny;
    ...
    function touchstarted() {
      ...
      let touches$$1 = exports.event.changedTouches;
        var c = container.apply(this, arguments);
        var n = touches$$1.length;
        var i;
        var gesture;
  ...
  const prototype = Object.create(parent.prototype);
    for (let key in definition) prototype[key] = definition[key];
    return prototype;
  }
...
const A = -0.14861;
  ...
  function cubehelixConvert(o) {
    ...
    let r = o.r / 255;
      ...
      var bl = b - l;
      var k = (E * (g - l) - C * bl) / D;
      var s = Math.sqrt(k * k + bl * bl) / (E * l * (1 - l)); // NaN if l=0 or l=1
  ...
  let interpolateRgb = (function rgbGamma(y) {
    let color$$1 = gamma(y);

    function rgb$$1(start, end) {
      let r = color$$1((start = rgb(start)).r, (end = rgb(end)).r);
...
const reA = /[-+]?(?:\d+\.?\d*|\.?\d+)(?:[eE][-+]?\d+)?/g;
  let reB = new RegExp(reA.source, "g");
...
const degrees = 180 / Math.PI;
  ...
  function decompose(a, b, c, d, e, f) {
    let scaleX; var scaleY; var skewX;
...
const rho = Math.SQRT2;
  ...
  function hsl$1(hue$$1) {
    return function (start, end) {
      let h = hue$$1((start = hsl(start)).h, (end = hsl(end)).h);
...
const hslLong = hsl$1(nogamma);
  ...
  function lab$1(start, end) {
    let l = nogamma((start = lab(start)).l, (end = lab(end)).l);
  ...
  function hcl$1(hue$$1) {
    return function (start, end) {
      let h = hue$$1((start = hcl(start)).h, (end = hcl(end)).h);
  ...
  function cubehelix$1(hue$$1) {
    return (function cubehelixGamma(y) {
      ...
      function cubehelix$$1(start, end) {
        let h = hue$$1((start = cubehelix(start)).h, (end = cubehelix(end)).h);
        var s = nogamma(start.s, end.s);
        var l = nogamma(start.l, end.l);
        var opacity = nogamma(start.opacity, end.opacity);
        return function (t) {
...
const cubehelixLong = cubehelix$1(nogamma);
  ...
  var timeout = 0; // is a timeout pending?
  var interval = 0; // are any timers active?
  var pokeDelay = 1000; // how frequently we check for clock skew
  var taskHead;
  var taskTail;
  var clockLast = 0;
  var clockNow = 0;
...
const ENDED = 6;
  ...
  function tweenRemove(id, name) {
    let tween0; var tween1;
    return function () {
      let schedule$$1 = set$1(this, id);
        var { tween } = schedule$$1;
  ...
  function attrConstant$1(name, interpolate$$1, value1) {
    let value00; var interpolate0;
    return function () {
      let value0 = this.getAttribute(name);
  ...
  function attrConstantNS$1(fullname, interpolate$$1, value1) {
    let value00; var interpolate0;
    return function () {
      let value0 = this.getAttributeNS(fullname.space, fullname.local);
      return value0 === value1
        ? null
        : value0 === value00
        ? interpolate0
  ...
  function attrFunctionNS$1(fullname, interpolate$$1, value) {
    let value00; var value10; var interpolate0;
    return function () {
      let value0;
      var value1 = value(this);
      if (value1 == null)
  ...
  function transition_attr(name, value) {
    let fullname = namespace(name);
      var i = fullname === "transform" ? interpolateTransformSvg : interpolate;
  ...
  function attrTweenNS(fullname, value) {
    function tween() {
      let node = this;
        var i = value.apply(node, arguments);
      return (
        i &&
        function (t) {
  ...
  function onFunction(id, name, listener) {
    let on0;
      var on1;
      var sit = start(name) ? init : set$1;
    return function () {
      let schedule$$1 = sit(this, id);
        var { on } = schedule$$1;
...
const Selection$1 = selection.prototype.constructor;
  ...
  function styleRemove$1(name, interpolate$$1) {
    ...
    var interpolate0;
    return function () {
      let value0 = styleValue(this, name);
      let value1 = (this.style.removeProperty(name), styleValue(this, name));
      return value0 === value1
        ? null
        : value0 === value00 && value1 === value10
        ? interpolate0
...
const exponent = 3;
  ...
  let polyInOut = (function custom(e) {
    e = +e;
...
const overshoot = 1.70158;
  ...
  let backInOut = (function custom(s) {
    s = +s;
...
const tau = 2 * Math.PI;
  ...
  let elasticIn = (function custom(a, p) {
  ...
  let elasticOut = (function custom(a, p) {
    let s = Math.asin(1 / (a = Math.max(1, a))) * (p /= tau);

    function elasticOut(t) {
      return 1 - a * 2**(-10 * (t = +t)) * Math.sin((t + s) / p);
    }
  ...
  let elasticInOut = (function custom(a, p) {
    let s = Math.asin(1 / (a = Math.max(1, a))) * (p /= tau);

    function elasticInOut(t) {
      return (
        ((t = t * 2 - 1) < 0
          ? a * 2**(10 * t) * Math.sin((s - t) / p)
          : 2 - a * 2**(-10 * t) * Math.sin((s + t) / p)) / 2
      );
    }
...
const MODE_DRAG = {name: "drag"};
  ...
  function brush$1(dim) {
    let extent = defaultExtent;
      ...
      var listeners = dispatch(brush, "start", "brush", "end");
      var handleSize = 6;
      var touchending;
    ...
    function brush(group) {
      ...
      let handle = group.selectAll(".handle").data(dim.handles, function (d) {
        return d.type;
      });
      ...
      if (group.selection) {
        group
          ...
          .tween("brush", function () {
            ...
            var selection1 = dim.input(
              typeof selection$$1 === "function"
                ? selection$$1.apply(this, arguments)
                : selection$$1,
              state.extent
            );
            var i = interpolateValue(selection0, selection1);
    ...
    const group = select(this);
      let selection$$1 = local$1(this).selection;
    ...
    function started() {
      ...
      let that = this;
        var {type} = exports.event.target.__data__;
        var mode =
        ...
        var state = local$1(that);
        var {extent} = state;
        var selection$$1 = state.selection;
        var W = extent[0][0];
        var w0;
        var w1;
        var N = extent[0][1];
        ...
        var dy;
        var moving;
        var shifting = signX && signY && exports.event.shiftKey;
        var lockX;
        var lockY;
        var point0 = mouse(that);
        var point$$1 = point0;
      ...
      let group = select(that).attr("pointer-events", "none");
    ...
    function initialize() {
      let state = this.__brush || { selection: null };
...
const pi$2 = Math.PI;
  var tau$2 = 2 * pi$2;
  var epsilon$1 = 1e-6;
  let tauEpsilon = tau$2 - epsilon$1;
  ...
  function ribbon() {
    ...
    function ribbon() {
      let buffer;
        var argv = slice$2.call(arguments);
        var s = source.apply(this, argv);
        var t = target.apply(this, argv);
        var sr = +radius.apply(this, ((argv[0] = s), argv));
        var sa0 = startAngle.apply(this, argv) - halfPi$1;
        var sa1 = endAngle.apply(this, argv) - halfPi$1;
        var sx0 = sr * cos(sa0);
        var sy0 = sr * sin(sa0);
        var tr = +radius.apply(this, ((argv[0] = t), argv));
        var ta0 = startAngle.apply(this, argv) - halfPi$1;
        var ta1 = endAngle.apply(this, argv) - halfPi$1;
...
const proto = map$1.prototype;
  ...
  function entries(map) {
    let entries = [];
    for (let key in map) entries.push({ key, value: map[key] });
    return entries;
  }
...
const slice$3 = array$2.slice;
  ...
  function contours() {
    let dx = 1;
      var dy = 1;
      var threshold$$1 = thresholdSturges;
      var smooth = smoothLinear;

    function contours(values) {
      let tz = threshold$$1(values);

      // Convert number of thresholds into uniform thresholds.
      if (!Array.isArray(tz)) {
      const domain = extent(values); let start = domain[0]; let stop = domain[1];
    ...
    contours.size = function (_) {
      ...
      let _0 = Math.ceil(_[0]);
        var _1 = Math.ceil(_[1]);
  ...
  function density() {
    let x = defaultX;
      ...
      var dx = 960;
      var dy = 500;
      var r = 20; // blur radius
      var k = 2; // log2(grid cell size)
      var o = r * 3; // grid offset, to pad for blur
      var n = (dx + o * 2) >> k; // grid width
      var m = (dy + o * 2) >> k; // grid height
      var threshold$$1 = constant$6(20);

    function density(data) {
      let values0 = new Float32Array(n * m);
        var values1 = new Float32Array(n * m);

      data.forEach(function (d, i, data) {
        let xi = (+x(d, i, data) + o) >> k;
          var yi = (+y(d, i, data) + o) >> k;
          var wi = +weight(d, i, data);
      ...
      let tz = threshold$$1(values0);

      // Convert number of thresholds into uniform thresholds.
      if (!Array.isArray(tz)) {
        let stop = max(values0);
    ...
    function transform(geometry) {
      geometry.value *= 2**(-2 * k); // Density in points per square pixel.
...
const EOL = {};
  ...
  function customConverter(columns, f) {
    let object = objectConverter(columns);
  ...
  function dsvFormat(delimiter) {
    let reFormat = new RegExp(`["${  delimiter  }\n\r]`);
...
const initialRadius = 10;
  ...
  function simulation(nodes) {
    let simulation;
      var alpha = 1;
      var alphaMin = 0.001;
      var alphaDecay = 1 - alphaMin**(1 / 300);
      var alphaTarget = 0;
      var velocityDecay = 0.6;
      var forces = map$1();
      var stepper = timer(step);
      var event = dispatch("tick", "end");
    ...
    function initializeNodes() {
      for (var i = 0, n = nodes.length, node; i < n; ++i) {
        ...
        if (isNaN(node.x) || isNaN(node.y)) {
          let radius = initialRadius * Math.sqrt(i);
...
const prefixes = ["y","z","a","f","p","n","\xB5","m","","k","M","G","T","P","E","Z","Y"];
  ...
  function formatLocale(locale) {
    let group =
      ...
      var {currency} = locale;
      var {decimal} = locale;
      var numerals = locale.numerals ? formatNumerals(locale.numerals) : identity$3;
    ...
    function newFormat(specifier) {
      ...
      let formatType = formatTypes[type];
...
const {tan} = Math;
  ...
  var streamGeometryType = {
    ...
    let {geometries} = object, i = -1, n = geometries.length;
...
const areaSum = adder();
  ...
  function areaPoint(lambda, phi) {
    ...
    let dLambda = lambda - lambda0;
      ...
      var u = cosPhi0 * cosPhi + k * cos$1(adLambda);
      var v = k * sdLambda * sin$1(adLambda);
  ...
  var lambda1;
  var phi1; // bounds
  var lambda2; // previous lambda-coordinate
  var lambda00$1;
  var phi00$1; // first point
  var p0; // previous 3D point
  var deltaSum = adder();
  var ranges;
  let range;
  ...
  function linePoint(lambda, phi) {
    ...
    if (p0) {
      ...
      let delta = lambda - lambda2;
        ...
        var lambdai = inflection[0] * degrees$1 * sign$$1;
        var phii;
      ...
      if (antimeridian) {
        if (lambda < lambda2) {
          if (angle(lambda0$1, lambda) > angle(lambda0$1, lambda1))
            lambda1 = lambda;
        } else if (angle(lambda, lambda1) > angle(lambda0$1, lambda1))
            lambda0$1 = lambda;
  ...
  function bounds(feature) {
    let i; var n; var a; var b; var merged; var deltaMax; var delta;
  ...
  function centroidLinePoint(lambda, phi) {
    (lambda *= radians), (phi *= radians);
    var cosPhi = cos$1(phi);
      var x = cosPhi * cos$1(lambda);
      var y = cosPhi * sin$1(lambda);
      var z = sin$1(phi);
      var w = atan2(
        sqrt(
          (w = y0 * z - z0 * y) * w +
  ...
  function centroidRingPoint(lambda, phi) {
    ...
    let cosPhi = cos$1(phi);
      ...
      var w = asin(m); // line weight = angle
  ...
  function rotationPhiGamma(deltaPhi, deltaGamma) {
    let cosDeltaPhi = cos$1(deltaPhi);
      var sinDeltaPhi = sin$1(deltaPhi);
      var cosDeltaGamma = cos$1(deltaGamma);
      var sinDeltaGamma = sin$1(deltaGamma);

    function rotation(lambda, phi) {
      let cosPhi = cos$1(phi);
        var x = cos$1(lambda) * cosPhi;
        var y = sin$1(lambda) * cosPhi;
        var z = sin$1(phi);
  ...
  function circleStream(stream, radius, delta, direction, t0, t1) {
    if (!delta) return;
    let cosRadius = cos$1(radius);
      var sinRadius = sin$1(radius);
  ...
  function circle() {
    let center = constant$8([0, 0]);
      var radius = constant$8(90);
      var precision = constant$8(6);
      var ring;
      var rotate;
      var stream = { point };
...
const sum$1 = adder();
  ...
  function polygonContains(polygon, point) {
    let lambda = point[0];
      var phi = point[1];
      var sinPhi = sin$1(phi);
      var normal = [sin$1(lambda), -cos$1(lambda), 0];
      var angle = 0;
      var winding = 0;
    ...
    for (let i = 0, n = polygon.length; i < n; ++i) {
      ...
      for (
        ...
        var point1 = ring[j];
          ...
          var antimeridian = absDelta > pi$3;
  ...
  function clipAntimeridianIntersect(lambda0, phi0, lambda1, phi1) {
    let cosPhi0;
      var cosPhi1;
      var sinLambda0Lambda1 = sin$1(lambda0 - lambda1);
  ...
  function clipAntimeridianInterpolate(from, to, direction, stream) {
    ...
    if (from == null) {
      ...
      let lambda = from[0] < to[0] ? pi$3 : -pi$3;
  ...
  function clipCircle(radius) {
    let cr = cos$1(radius);
      ...
      var notHemisphere = abs(cr) > epsilon$2; // TODO optimise for this common case
    ...
    function clipLine(stream) {
      let point0; // previous point
        var c0; // code for previous point
        var v0; // visibility of previous point
        var v00; // visibility of first point
        var clean; // no intersections
      return {
        lineStart () {
          v00 = v0 = false;
    ...
    function intersect(a, b, two) {
      ...
      let n1 = [1, 0, 0]; // normal
        ...
        var n1n2 = n2[0]; // cartesianDot(n1, n2),
      ...
      let delta = lambda1 - lambda0;
        var polar = abs(delta - pi$3) < epsilon$2;
        var meridian = polar || delta < epsilon$2;
...
const clipMax = 1e9; let clipMin = -clipMax;
    ...
    return function (stream) {
      ...
      function polygonEnd() {
        let startInside = polygonInside();
          ...
          var visible = (segments = merge(segments)).length;
...
const lengthSum = adder();
  ...
  function lengthPoint(lambda, phi) {
    (lambda *= radians), (phi *= radians);
    let sinPhi = sin$1(phi);
      var cosPhi = cos$1(phi);
      var delta = abs(lambda - lambda0$2);
      var cosDelta = cos$1(delta);
      var sinDelta = sin$1(delta);
...
const coordinates = [null, null];
  let object$1 = { type: "LineString", coordinates };
  ...
  function containsLine(coordinates, point) {
    let ab = distance(coordinates[0], coordinates[1]);
  ...
  function graticuleX(y0, y1, dy) {
    let y = sequence(y0, y1 - epsilon$2, dy).concat(y1);
    return function (x) {
  ...
  function interpolate$1(a, b) {
    let x0 = a[0] * radians;
      ...
      var cy1 = cos$1(y1);
      var sy1 = sin$1(y1);
      var kx0 = cy0 * cos$1(x0);
      var ky0 = cy0 * sin$1(x0);
      var kx1 = cy1 * cos$1(x1);
      var ky1 = cy1 * sin$1(x1);
...
const areaSum$1 = adder();
  ...
  var centroidStream$1 = {
    ...
    result () {
    let centroid = Z2$1 ? [X2$1 / Z2$1, Y2$1 / Z2$1]
        : Z1$1
        ? [X1$1 / Z1$1, Y1$1 / Z1$1]
        : Z0$1
        ? [X0$1 / Z0$1, Y0$1 / Z0$1]
...
const maxDepth = 16; // maximum depth of subdivision
  let cosMinDistance = cos$1(30 * radians); // cos(minimum angular distance)
  ...
  function scaleTranslateRotate(k, dx, dy, alpha) {
    let cosAlpha = cos$1(alpha);
      var sinAlpha = sin$1(alpha);
      var a = cosAlpha * k;
      var b = sinAlpha * k;
      var ai = cosAlpha / k;
      var bi = sinAlpha / k;
      var ci = (sinAlpha * dy - cosAlpha * dx) / k;
      var fi = (sinAlpha * dx + cosAlpha * dy) / k;
    function transform$$1(x, y) {
  ...
  function projectionMutator(projectAt) {
    ...
    var deltaPhi = 0;
    var deltaGamma = 0;
    var rotate; // pre-rotate
    var alpha = 0; // post-rotate
    var theta = null;
    var preclip = clipAntimeridian; // pre-clip angle
    var x0 = null;
    var y0;
    var x1;
    var y1;
    var postclip = identity$4; // post-clip extent
    var delta2 = 0.5; // precision
    var projectResample;
    var projectTransform;
    var projectRotateTransform;
    var cache;
    let cacheStream;
    ...
    function recenter() {
      let center = scaleTranslateRotate(k, 0, 0, alpha).apply(
          null,
          project(lambda, phi)
        );
        var transform$$1 = (alpha ? scaleTranslateRotate : scaleTranslate)(
          k,
          x - center[0],
          y - center[1],
          alpha
  ...
  function albersUsa() {
    let cache;
      ...
      var alaska = conicEqualArea()
        ...
        .center([-2, 58.5])
        .parallels([55, 65]);
      var alaskaPoint; // EPSG:3338
      var hawaii = conicEqualArea()
        .rotate([157, 0])
        .center([-3, 19.9])
        .parallels([8, 18]);
      var hawaiiPoint; // ESRI:102007
      var point;
      var pointStream = {
        point (x, y) {
          point = [x, y];
  ...
  let azimuthalEqualAreaRaw = azimuthalRaw(function (cxcy) {
  ...
  let azimuthalEquidistantRaw = azimuthalRaw(function (c) {
  ...
  function conicConformalRaw(y0, y1) {
    ...
    project.invert = function (x, y) {
    const fy = f - y; let r = sign(n) * sqrt(x * x + fy * fy);
      return [
        (atan2(x, abs(fy)) / n) * sign(fy),
        2 * atan((f / r)**(1 / n)) - halfPi$2,
      ];
    };
  ...
  function conicEquidistantRaw(y0, y1) {
    let cy0 = cos$1(y0);
      var n = y0 === y1 ? sin$1(y0) : (cy0 - cos$1(y1)) / (y1 - y0);
      var g = cy0 / n + y0;

    if (abs(n) < epsilon$2) return equirectangularRaw;

    function project(x, y) {
    const gy = g - y; let nx = n * x;
...
const A1 = 1.340264;
  ...
  const l = asin(M * sin$1(phi)); let l2 = l * l; let l6 = l2 * l2 * l2;
    return [
      (lambda * cos$1(l)) /
  ...
  function identity$5() {
    let k = 1;
      ...
      var sx = 1;
      var sy = 1;
      var transform$$1 = identity$4; // scale, translate and reflect
      var x0 = null;
      var y0;
      var x1;
      var y1; // clip extent
      var postclip = identity$4;
      var cache;
      var cacheStream;
      var projection;
  ...
  function cluster() {
    ...
    const left = leafLeft(root);
      ...
      let x1 = right.x + separation(right, left) / 2;
...
const slice$4 = Array.prototype.slice;
  ...
  function enclose(circles) {
    let i = 0;
      var n = (circles = shuffle$1(slice$4.call(circles))).length;
      var B = [];
      var p;
      var e;
  ...
  function packEnclose(circles) {
    ...
    let a; var b; var c; var n; var aa; var ca; var i; var j; var k; var sj; var sk;
  ...
  const nodes = parent.children;
    ...
    var i = -1;
    var n = nodes.length;
    let k = parent.value && (x1 - x0) / parent.value;
...
const phi = (1 + Math.sqrt(5)) / 2;
  ...
  let squarify = (function custom(ratio) {
  ...
  let resquarify = (function custom(ratio) {
  ...
  let uniform = (function sourceRandomUniform(source) {
  ...
  let normal = (function sourceRandomNormal(source) {
  ...
  let logNormal = (function sourceRandomLogNormal(source) {
    function randomLogNormal() {
      let randomNormal = normal.source(source).apply(this, arguments);
      return function () {
  ...
  let irwinHall = (function sourceRandomIrwinHall(source) {
  ...
  let bates = (function sourceRandomBates(source) {
    function randomBates(n) {
      let randomIrwinHall = irwinHall.source(source)(n);
      return function () {
  ...
  let exponential$1 = (function sourceRandomExponential(source) {
...
const implicit = {name: "implicit"};
  ...
  function band() {
    ...
    const n = domain().length;
      ...
      let values = sequence(n).map(function (i) {
        return start + step * i;
      });
...
const unit = [0, 1];
  ...
  function polymap(domain, range, deinterpolate, reinterpolate) {
    ...
    return function (x) {
      let i = bisectRight(domain, x, 1, j) - 1;
      return r[i](d[i](x));
    };
  }
  ...
  function continuous(deinterpolate, reinterpolate) {
    let domain = unit;
    var range = unit;
    var interpolate$$1 = interpolateValue;
    var clamp = false;
    var piecewise$$1;
    var output;
    let input;
  ...
  const start = domain[0];
    ...
    var step = tickStep(start, stop, count == null ? 10 : count);
    let precision;
    specifier = formatSpecifier(specifier == null ? ",f" : specifier);
    switch (specifier.type) {
      case "s": {
        let value = Math.max(Math.abs(start), Math.abs(stop));
        if (
  ...
  function linear$2() {
    let scale = continuous(deinterpolateLinear, interpolateNumber);
  ...
  function log$1() {
    let scale = continuous(deinterpolate, reinterpolate).domain([1, 10]);
      var {domain} = scale;
      var base = 10;
      var logs = logp(10);
      var pows = powp(10);
...
const minutes = minute.range;
  ...
  let hour = newInterval(
    function (date) {
      let offset = (date.getTimezoneOffset() * durationMinute) % durationHour;
...
const utcYears = utcYear.range;
  ...
  function localDate(d) {
    if (d.y >= 0 && d.y < 100) {
      let date = new Date(-1, d.m, d.d, d.H, d.M, d.S, d.L);
  ...
  function formatLocale$1(locale) {
    let locale_dateTime = locale.dateTime;
  ...
  const periodRe = formatRe(locale_periods);
    ...
    let shortMonthLookup = formatLookup(locale_shortMonths);
    ...
    function parsePeriod(d, string, i) {
      let n = periodRe.exec(string.slice(i));
      return n
    ...
    function parseShortWeekday(d, string, i) {
      let n = shortWeekdayRe.exec(string.slice(i));
      return n
    ...
    function parseWeekday(d, string, i) {
      let n = weekdayRe.exec(string.slice(i));
      return n
    ...
    function parseShortMonth(d, string, i) {
      let n = shortMonthRe.exec(string.slice(i));
      return n
    ...
    function parseMonth(d, string, i) {
      let n = monthRe.exec(string.slice(i));
      return n
    ...
    return {
      ...
      utcParse (specifier) {
      let p = newParse(specifier, utcDate);
        p.toString = function () {
          return specifier;
        };
        return p;
  ...
  const map = {}; let i = -1; let n = names.length;
    while (++i < n) map[names[i].toLowerCase()] = i;
    return map;
  }

  function parseWeekdayNumberSunday(d, string, i) {
    let n = numberRe.exec(string.slice(i, i + 1));
  ...
  function parseWeekNumberISO(d, string, i) {
    let n = numberRe.exec(string.slice(i, i + 2));
  ...
  function parseFullYear(d, string, i) {
    let n = numberRe.exec(string.slice(i, i + 4));
  ...
  function parseZone(d, string, i) {
    let n = /^(Z)|([+-]\d\d)(?::?(\d\d))?/.exec(string.slice(i, i + 6));
    return n
  ...
  function parseDayOfYear(d, string, i) {
    let n = numberRe.exec(string.slice(i, i + 3));
  ...
  function parseMicroseconds(d, string, i) {
    let n = numberRe.exec(string.slice(i, i + 6));
    return n ? ((d.L = Math.floor(n[0] / 1000)), i + n[0].length) : -1;
  }

  function parseLiteralPercent(d, string, i) {
    let n = percentRe.exec(string.slice(i, i + 1));
    return n ? i + n[0].length : -1;
  }

  function parseUnixTimestamp(d, string, i) {
    let n = numberRe.exec(string.slice(i));
...
const isoSpecifier = "%Y-%m-%dT%H:%M:%S.%LZ";
  ...
  let formatIso = Date.prototype.toISOString
    ? formatIsoNative
  ...
  let parseIso = +new Date("2000-01-01T00:00:00.000Z")
    ? parseIsoNative
...
const durationSecond$1 = 1000;
  ...
  let durationYear = durationDay$1 * 365;
  ...
  function calendar(
    ...
    let formatMillisecond = format(".%L");
  ...
  const n = specifier.length / 6 | 0; let colors = new Array(n); let i = 0;
...
const tau$4 = 2 * pi$4;
  ...
  function cornerTangents(x0, y0, x1, y1, r1, rc, cw) {
    let x01 = x0 - x1;
      var y01 = y0 - y1;
      var lo = (cw ? rc : -rc) / sqrt$2(x01 * x01 + y01 * y01);
      var ox = lo * y01;
      var oy = -lo * x01;
      var x11 = x0 + ox;
      var y11 = y0 + oy;
      ...
      var d = (dy < 0 ? -1 : 1) * sqrt$2(max$2(0, r * r * d2 - D * D));
  ...
  function arc() {
    ...
    function arc() {
      let buffer;
        ...
        var a0 = startAngle.apply(this, arguments) - halfPi$3;
        var a1 = endAngle.apply(this, arguments) - halfPi$3;
        var da = abs$1(a1 - a0);
        var cw = a1 > a0;
      ...
      // Or is it a circular or annular sector?
        ...
        let a01 = a0;
          ...
          var rp =
            ...
            (padRadius
              ...
              : sqrt$2(r0 * r0 + r1 * r1));
          var rc = min$1(abs$1(r1 - r0) / 2, +cornerRadius.apply(this, arguments));
          var rc0 = rc;
          var rc1 = rc;
          var t0;
          var t1;
        ...
        let x01 = r1 * cos$2(a01);
          var y01 = r1 * sin$2(a01);
          var x10 = r0 * cos$2(a10);
          var y10 = r0 * sin$2(a10);
        ...
        if (rc > epsilon$3) {
          ...
          if (da < pi$4) {
            let oc =
              ...
              var lc = sqrt$2(oc[0] * oc[0] + oc[1] * oc[1]);
  ...
  function pie() {
    let value = identity$7;
      var sortValues = descending$1;
      var sort = null;
      var startAngle = constant$b(0);
      var endAngle = constant$b(tau$4);
      var padAngle = constant$b(0);
...
const curveRadialLinear = curveRadial(curveLinear);
  ...
  function areaRadial() {
    let a = area$3().curve(curveRadialLinear);
      var c = a.curve;
      var x0 = a.lineX0;
      var x1 = a.lineX1;
      var y0 = a.lineY0;
...
const slice$6 = Array.prototype.slice;
  ...
  function link$2(curve) {
    ...
    function link() {
      let buffer;
        var argv = slice$6.call(arguments);
        var s = source.apply(this, argv);
        var t = target.apply(this, argv);
      if (!context) context = buffer = path();
      curve(
...
const symbols = [
  ...
  let bundle = (function custom(beta) {
  ...
  let cardinal = (function custom(tension) {
  ...
  let cardinalClosed = (function custom(tension) {
  ...
  let cardinalOpen = (function custom(tension) {
    function cardinal$$1(context) {
  ...
  let catmullRom = (function custom(alpha) {
  ...
  let catmullRomClosed = (function custom(alpha) {
  ...
  let catmullRomOpen = (function custom(alpha) {
    function catmullRom$$1(context) {
      return alpha
  ...
  function stack() {
    ...
    function stack(data) {
      let kz = keys.apply(this, arguments);
        var i;
        var m = data.length;
        var n = kz.length;
        var sz = new Array(n);
  ...
  const n = series.length;
    ...
    var order = none$2(series).sort(function (a, b) {
  ...
  Diagram.prototype = {
    ...
    find (x, y, radius) {
      ...
      do {
        ...
        let edge = that.edges[e]; var v = edge.left;
...
const identity$8 = new Transform(1, 0, 0);
  ...
  function defaultConstrain(transform, extent, translateExtent) {
    let dx0 = transform.invertX(extent[0][0]) - translateExtent[0][0];
      ...
      var dy1 = transform.invertY(extent[1][1]) - translateExtent[1][1];
  ...
  function zoom() {
    let filter = defaultFilter$2;
      ...
      var touchable = defaultTouchable$1;
      ...
      var duration = 250;
      var interpolate = interpolateZoom;
      var gestures = [];
    ...
    zoom.transform = function (collection, transform) {
      let selection$$1 = collection.selection
    ...
    zoom.scaleBy = function (selection$$1, k) {
      ...
      const k0 = this.__zoom.k;
        let k1 = typeof k === "function" ? k.apply(this, arguments) : k;
        return k0 * k1;
      });
    };
    ...
    function mousedowned() {
      ...
      let g = gesture(this, arguments);
        var v = select(exports.event.view)
          .on("mousemove.zoom", mousemoved, true)
          .on("mouseup.zoom", mouseupped, true);
        var p = mouse(this);
        var x0 = exports.event.clientX;
        var y0 = exports.event.clientY;
      ...
      function mousemoved() {
        noevent$2();
        if (!g.moved) {
          let dx = exports.event.clientX - x0;
            var dy = exports.event.clientY - y0;
    ...
    function dblclicked() {
      ...
      let t0 = this.__zoom;
        var p0 = mouse(this);
        var p1 = t0.invert(p0);
        var k1 = t0.k * (exports.event.shiftKey ? 0.5 : 2);
        var t1 = constrain(
