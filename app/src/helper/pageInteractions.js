...
export function openActivity() {
  const slide = document.getElementsByClassName("LA")[0];
  slide.setAttribute("style", "grid-template-columns: 20vw 50vw;");

  const ref = document.getElementsByClassName("lesson")[1];
  ref.classList.remove("hide");

  const lesson = document.getElementsByClassName("lesson")[0];
  lesson.classList.add("hide");

  const actPrompt = document.getElementsByClassName("activity")[0];
  actPrompt.classList.add("hide");

  const activity = document.getElementsByClassName("activity")[1];
...
export function newTest(lessonNum) {
  ...
  if (lessonNum == 1) {
    if (Math.random() > 0.5) {
      df.presentOption(
        "firstResponse",
        df.pickIncorrectOption(thisTest),
        thisTest
      );
    ...
    df.addEvaluator(
      document.getElementById("firstResponse").firstChild.id,
      thisTest
    );
    df.addEvaluator(
      document.getElementById("secondResponse").firstChild.id,
      thisTest
    );
  }
  if (lessonNum == 2) {
    ...
    df.addEvaluator(
      document.getElementById("thirdResponse").firstChild.id,
      thisTest
    );
    df.addEvaluator(
      document.getElementById("fourthResponse").firstChild.id,
      thisTest
    );
  }
  if (lessonNum == 3) {
    const csvFile = testsArray[thisTestNumber].id.replace(/[0-9]/g, "");
    drawf.drawNativePitchCurve(
      `../assets/sounds/huanglaoshi/csv/${csvFile}.csv`,
      750,
      350,
      225,
      100,
    ...
    df.showCharacter(
      "character",
      "../js/huanglaoshiTestsShortened.json",
      thisTest.character,
      thisTest.pinyin
    );
  }
  ...
  if (lessonNum == 3 && shownTests.length == testsArray.length) {
    ...
    const newContButton = continueButton.cloneNode(true);
...
export function advance(lessonNum) {
  document.getElementById(`activity-${lessonNum}-baseline`).style.display =
    "none";
  document.getElementById(`activity-${lessonNum}-content`).style.display =
    "block";
